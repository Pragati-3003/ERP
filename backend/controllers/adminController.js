...
const addStudent = async (req, res) => {
  try {
    ...
    if (!department) {
      return res
        .status(400)
        .json({ message: "Invalid department: department does not exist" });
    }
  ...
  try {
    ...
    if (!Email || !FirstName || !LastName) {
      return res.status(400).json({
        message: "Missing required fields: Email, FirstName, or LastName",
      });
    }
...
const addCurriculum = async (req, res) => {
  try {
    const { program, specialization, deptId, semesters } = req.body;

    // ✅ Check if curriculum already exists for the same program + specialization
    const existing = await Curriculum.findOne({ program, specialization });
    if (existing) {
      return res.status(400).json({
        message: "Curriculum already exists for this Program & Specialization",
      });
    }
    ...
    res
      .status(201)
      .json({ message: "Curriculum added successfully", curriculum });
  } catch (err) {
...
const addStudentTimeTable = async (req, res) => {
  try {
    ...
    if (timetable) {
      ...
      return res
        .status(201)
        .json({ message: "Timetable added successfully", timetable });
    }
  } catch (err) {
...
const addCourseToTeacher = async (req, res) => {
  try {
    const { teacherEmail, courseCode, courseName, program, specialization } =
      req.body;
    // console.log(req.body)
    ...
    const isAlreadyAssigned = teacher.CoursesTaught.some(
      (taught) =>
        taught.course.equals(course._id) &&
        taught.curriculum.equals(curriculum._id)
    );

    if (!isAlreadyAssigned) {
      teacher.CoursesTaught.push({
        course: course._id,
    ...
    res
      .status(200)
      .json({ message: "Course assigned to teacher successfully", teacher });
  } catch (error) {
...
const getAdminProfilebyEmail = async (req, res) => {
  try {
    ...
    if (!admin) return res.status(404).json({ message: "Admin doesm't exist" });
    const updatedAdmin = {
      Name: admin.Name || "",
...
const getCurriculum = async (req, res) => {
  ...
  try {
    const curriculum = await Curriculum.findOne({ program, specialization })
      ...
      .populate("semesters.courses", "CourseName");
...
const addSemesterResult = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error adding result", error: error.message });
  }
};
module.exports = {
  addSemesterResult,
