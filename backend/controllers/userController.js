...
const getStudentByCourse = async (req, res) => {
  try {
    ...
    if (!program) {
      return res
        .status(400)
        .json({ message: "Program name is required (e.g., MCA)" });
    }
    ...
    if (!curriculums) {
      return res.status(404).json({
        message: "No curriculum found for this course in the specified program",
      });
    }
    ...
    if (!students.length) {
      return res.status(404).json({
        message: "No students found for this course in the specified program",
      });
    }
...
const getAttendance = async (req, res) => {
  try {
    ...
    if (!curriculum.semesters || curriculum.semesters.length === 0) {
      return res
        .status(404)
        .json({ message: "No semesters found in curriculum" });
    }
    ...
    if (!currentSemester) {
      return res
        .status(404)
        .json({ message: "Semester not found in curriculum" });
    }
...
const getAllCoursesByCurriculumID = async (req, res) => {
  try {
    ...
    const semestersDetail = curriculum.semesters.find(
...
const getSemesterResultByStudentId = async (req, res) => {
  try {
    ...
    const result = await EndSemesterResult.findOne({
      StudentSmartID: smartCardId,
    });
    // console.log(result);
    if (!result)
...
module.exports = {
  getTeacherByEmail,
  getStudentsBySmartID,
  getSemesterResultByStudentId,
  getAllCoursesByCurriculumID,
  getAttendance,
  getStudentAttendanceOfParticularCourse,
  getCourseByNameandCode,
