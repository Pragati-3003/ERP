...
const AddTeacherCourse = () => {
  ...
  const fetchAssignedCourses = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/admin/get-courses-teacher/${teacherEmail}`,
  ...
  const handleAddCourse = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/admin/add-course-teacher",
        { teacherEmail, ...formData },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleUpdateCourse = async (courseId) => {
    ...
    try {
      await axios.put("http://localhost:5000/api/admin/update-course-teacher", {
        teacherEmail,
        courseId,
        newCourseCode,
      });
  ...
  const handleDeleteCourse = async (courseId) => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/admin/delete-course-teacher/${teacherEmail}/${courseId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
            ...
            {assignedCourses.length > 0 &&
              assignedCourses.map((course) => (
                ...
                >
                  ...
                  <span>
                    {course.curriculum?.program || "N/A"} (
                    {course.curriculum?.specialization || "N/A"})
                  </span>
                  <button
